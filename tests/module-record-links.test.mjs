...
test('record title renders a native link with encoded id and readable accessible name',()=>{const columns=moduleRecordColumns(module),html=render(columns.find(c=>c.id==='name'));assert.ok(html.includes('href="/clients?record='+encodeURIComponent(record.id)+'"'));assert.match(html,/<a /);assert.match(html,/data-workspace-nav="ignore"/,'record links must bypass query-only SPA interception');assert.match(html,/Client &amp; associé/);assert.doesNotMatch(html,/tabindex="-1"|role="button"/);});
