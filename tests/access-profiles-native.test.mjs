...
test('real D1 preserves additive migration, CAS winner and zero-effect stale invite',async()=>{
 ...
 try{
  const directory=fileURLToPath(new URL('../template/drizzle',import.meta.url));assert.equal(await migrate(local.db,directory),14);assert.equal(await migrate(local.db,directory),0);
