...
test('all native migrations apply on empty D1 and replay without modification',async()=>{
 ...
 try{
 assert.equal(await migrate(local.db,drizzleDir),13);
...
test('persistent local D1 keeps data and migration markers after restart',async()=>{
 ...
 assert.equal(await migrate(local.db,directory),13);
 ...
 assert.equal((await local.db.prepare('SELECT COUNT(*) AS n FROM d1_migrations').first()).n,13);
