...
test('Cloudflare sends through its REST API once, reports failures honestly and keeps secrets/content out of logs',async()=>{
  const {db,a}=await setup(),oldFetch=globalThis.fetch;try{
    const connection=await integration(a);let calls=0;globalThis.fetch=async(url,init)=>{calls++;assert.equal(url,`https://api.cloudflare.com/client/v4/accounts/${'a'.repeat(32)}/email/sending/send`);assert.equal(init.headers.Authorization,`Bearer ${secret}`);assert.equal(init.redirect,'error');const body=JSON.parse(init.body);assert.equal(body.from,'team@example.com');assert.equal(body.text,'Corps privé du message');return Response.json({success:true,result:{delivered:[],queued:body.to,permanent_bounces:[]}});};
