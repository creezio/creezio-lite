...
test('workerd: native UI relay crosses independent requests through real D1',async()=>{
  ...
  try{
    ...
    await db.prepare("INSERT INTO lite_browser_sessions(org_id,user_id,kind,window_id,lease_until,path) VALUES(?,?,'desktop','fixture-desktop-window',?,'/')").bind(org,'alice',new Date(Date.now()+60000).toISOString()).run();
    ...
    assert.equal((await mf.dispatchFetch(`${endpoint}/claim?workspace=${org}`,{method:'POST',headers:{'x-lite-window':'fixture-desktop-window'}})).status,200);
    assert.equal((await mf.dispatchFetch(`${endpoint}/claim?workspace=${org}`,{method:'POST',headers:{'x-lite-window':'fixture-desktop-window'}})).status,409);
    assert.equal((await mf.dispatchFetch(`${endpoint}/result?workspace=${org}`,{method:'POST',headers:{'content-type':'application/json','x-lite-window':'fixture-desktop-window'},body:JSON.stringify({ok:true,clicked:'Mail'})})).status,200);
    let tail='';while(true){const r=await reader.read();if(r.done)break;tail+=decoder.decode(r.value);}
    assert.deepEqual(JSON.parse(tail),{event:'result',data:{ok:true,clicked:'Mail',actionId:first.data.actionId}});
    assert.equal(await db.prepare('SELECT count(*) n FROM lite_assistant_ui_actions').first('n'),1);
