...
- Distribution : `create` et `adopt` copient tout le dossier `.cursor/skills/lite-orchestration/` (récursivement : contrat, sélections, scripts, références, schémas, exemples), la règle `template/.cursor/rules/lite-orchestration.mdc`, et génèrent le point d’entrée de découverte Codex `.agents/skills/lite-orchestration/SKILL.md` (même nom et description, renvoi vers la copie canonique et liste des ressources distribuées) ; le tout est suivi par `manifest.json` (`files` et `generated`). Ajouter une ressource au standard = ajouter un fichier dans le dossier canonique ; aucune liste manuelle, aucun copier-coller, aucune copie `.agents` maintenue à la main (le kit lui-même n’en porte pas ; `AGENTS.md` et les règles restent les points d’entrée documentés de Codex et Cursor). `npm run check` vérifie cette cohérence ; `tests/orchestration-distribution.test.mjs` couvre distribution, découverte, conflits et confinement.
