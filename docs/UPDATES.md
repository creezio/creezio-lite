...
## Correction 0.13.1 — Frontmatter CRLF du CLI kit

Le défaut est dans le lecteur du kit (`bin/lite.mjs`), pas dans les copies déjà installées. Sur un checkout Windows avec `core.autocrlf=true`, `node bin/lite.mjs adopt --app APP` et `create` acceptent désormais le `SKILL.md` canonique en CRLF comme en LF. Utiliser le CLI 0.13.1 : un simple `adopt --apply` depuis ce checkout suffit à relire le frontmatter ; les onze fichiers du dossier de compétence, la règle et le point de découverte **ne changent pas d’empreinte** par rapport à 0.13.0 si le checkout kit est resté LF. Le manifeste d’orchestration peut seulement porter `kitVersion: 0.13.1`. L’upgrade habituel du runtime aligne les chaînes de version API/MCP (0.13.1) et le verrou ; aucune migration, aucun secret, aucune fusion de pages applicatives. Ne pas activer `core.autocrlf` dans le dépôt du kit : ce lot ne normalise pas les empreintes CRLF des copies.
