...
Capacité additive du standard d’orchestration, **non publiée** : `lite adopt --apply` met à jour `scripts/cursor-agents.mjs`, `CONTRACT.md` et `SKILL.md` depuis un checkout de cette branche. Reprend le skill 0.13.2 déjà publié. Le runtime métier, le schéma, les migrations et les secrets ne changent pas ; l’upgrade du runtime ne fait qu’aligner les chaînes de version API/MCP.
...
Constat d’un lancement réel (cause du 400 **non prouvée**) : source en SHA → `400 validation_error` alors que le catalogue listait les URL, puis GET propriétaire 404 → `not_created` ; un mono-repo cible SHA a échoué puis une cible branche a été acceptée avec le même agent après réconciliation. Les docs officielles autorisent un SHA. Une limite « SHA seulement sur branche default/PR » n’est pas établie pour une source. Le kit ne remplace pas le SHA par une branche, n’omet pas la source, n’ajoute aucune sonde ni nouvelle référence. Échec visible ≠ crédits épuisés. Une prochaine réception utile (orchestrateur) peut attacher un SHA **déjà présent sur la branche default** d’un dépôt source déjà approuvé au catalogue, pour distinguer cette contrainte d’un échec d’attachement. **Ne pas publier cette capacité** tant que le mainteneur n’a pas un appel accepté utile et un checkout observé. Version **pas encore livrée** tant que la release GitHub n’est pas publiée.
