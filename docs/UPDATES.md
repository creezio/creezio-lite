...
La sélection du modèle (`fable` par défaut ; `opus` ou `grok` pour une mission clairement plus simple et bornée) est choisie une fois à l’attribution, validée contre le catalogue puis conservée toute la mission ; les reprises n’envoient aucun champ `model`. La clé `CURSOR_API_KEY` n’est lue que dans l’environnement de l’orchestrateur ; elle n’est stockée ni dans l’application ni dans ses journaux.
