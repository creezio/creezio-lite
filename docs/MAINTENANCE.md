...
Une seule issue est créée par version, même après relance. Le workflow s'exécute toutes les 30 minutes, au changement du verrou ou manuellement ; GitHub peut retarder les runs. Les erreurs d'accès ou d'API échouent explicitement, sans annoncer une livraison réussie. La vérification continue même lorsque le PC du mainteneur est éteint.
