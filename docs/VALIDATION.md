...
## 0.8.1 — Origine du formulaire de consentement

Les logs de production ont confirmé un POST `/oauth/decision` avec `Origin: null`, `Sec-Fetch-Site: same-origin` et `Sec-Fetch-Mode: navigate`. Le document héritait de la politique globale `no-referrer`, qui rend l’origine opaque pour un POST de formulaire natif. La page de consentement définit désormais sa seule politique `same-origin`.

Le nouveau contrôle `node scripts/verify-oauth-consent.mjs [application]` charge le Worker compilé sous workerd/D1. Il échoue sur la version 0.8.0 (métadonnée `no-referrer`) et passe sur le template et l’application corrigés : métadonnée HTML effective, retour de connexion Sites, champs du formulaire, refus des origines nulles/étrangères, acceptation du POST interne, retour OAuth avec `iss`, échange PKCE et liste d’outils MCP. Ce contrôle HTTP avec identité de fixture est maintenant exécuté en CI après compilation ; il ne simule pas un navigateur et ne prétend pas reproduire la session du compte utilisateur. Les 50 tests existants, les typechecks et les compilations passent aussi.
