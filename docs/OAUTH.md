...
La page de consentement utilise `Referrer-Policy: same-origin` via ses métadonnées. La politique globale `no-referrer` provoquait `Origin: null` sur les navigations POST des formulaires, comme confirmé dans les logs de production. Cette exception de page conserve l’origine du POST interne et n’envoie pas de référent au client OAuth externe. Le serveur continue à refuser `Origin: null` ; aucun élargissement des origines autorisées n’est nécessaire. Voir le [contrat Fetch sur l’en-tête Origin](https://fetch.spec.whatwg.org/#append-a-request-origin-header).
