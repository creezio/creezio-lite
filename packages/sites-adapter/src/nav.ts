...
import type { AppDefinition } from '@creezio-lite/core';
...
export function nativeEntries(app: AppDefinition): NavCatalogEntry[] {
  ...
  return [extra('os.dashboard','/dashboard','Tableau de bord','LayoutDashboard',0),
    ...
    ...app.modules.map((m,i):NavCatalogEntry=>({id:`module.${m.id}`,href:`/${m.id}`,label:m.name,icon:'FileText',group:'brand',order:40+i,source:'module',available:true,defaultVisible:true,permission:`module.${m.id}.read`}))];
}
