...
import { fail } from '@creezio-lite/core/validation';
...
export function tasksMount(c:NativeContext):ApiMount {
  ...
  return {dbLayer:'brand',operations:[
    ...
    if(!id&&method==='POST'){
      ...
      if((b.executor_kind??'human')!=='human'||b.dispatch===true||b.launch===true||b.recurring_schedule)fail(422,'executor_unavailable','Cet exécuteur nécessite un hôte Creezio.');
    ...
    if(id==='sync'||parts.length>1)fail(422,'host_runner_unavailable','Cette opération nécessite un hôte Creezio ; aucun run n’a été lancé.');
