...
import type { ApiContext, Workspace } from '@creezio-lite/core';
import { handleApi } from '@creezio-lite/core';
import { workspace as getWorkspace } from '@creezio-lite/core/api';
import { ApiError, fail } from '@creezio-lite/core/validation';
import { checkOrigin, readJson, json } from '@creezio-lite/core/http';
...
export function workspaceCookie(request:Request):string|null {
  const value=request.headers.get('cookie')?.split(';').map(s=>s.trim()).find(s=>s.startsWith('creezio_workspace='))?.slice(18);
...
export async function handleNativeApi(request:Request,context:ApiContext):Promise<Response|null> {
  ...
  try{
    ...
    let org:Workspace;
    try {org=await getWorkspace(db,user,workspaceCookie(request));}catch(e){
    ...
    if(path==='workspaces/select'&&request.method==='POST'){const b=await readJson(request);const next=await getWorkspace(db,user,String(b.workspaceId??''));const r=json({workspace:next});r.headers.set('Set-Cookie',`creezio_workspace=${next.id}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=31536000`);return r;}
    ...
    const kernel=createApiKernel({brandId:context.app.id,appVersion:'0.2.0',authorizeModuleAccess:({permission})=>({allow:permissions(c,context.app).includes(permission),reason:'permission_denied'})});
    ...
    if(response.status<300 && ["POST","PUT","PATCH","DELETE"].includes(request.method))result.headers.set("x-creezio-data-changed",path.startsWith("modules/nav")?"nav":path.startsWith("tasks")?"tasks":"support");
    return result;
  }catch(e){if(e instanceof ApiError)return json({ok:false,error:e.message,code:e.code},e.status);console.error('Native Creezio request failed',e instanceof Error?e.name:'Error');return json({ok:false,error:'Erreur du service Creezio.',code:'internal_error'},500);}
}
