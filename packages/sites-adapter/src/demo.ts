import { createInteractiveDemoMount, genericOsTourScenario, type DemoScenarioOverride } from '@creezio/interactive-demo';
...
export function demoMount(c:NativeContext,productName:string):ApiMount {
  ...
  const mount=createInteractiveDemoMount({defaults:[scenario],persistence:{
