...
export async function handleApi(request: Request, context: ApiContext, options: { beforeWrite?: BeforeWrite } = {}): Promise<Response> {
  ...
  try {
    ...
    if (path === 'health' && request.method === 'GET') {
      ...
      return json({ok:true,kit:'creezio-lite',version:'0.2.2',database:'ready'});
    }
    ...
    if (recordMatch) {
      const mod=context.app.modules.find(m=>m.id===recordMatch[1]); if(!mod) fail(404,'module_not_found','Module introuvable.');
      ...
      if(request.method==='GET') {
        ...
        if(q){where+=" AND search_text LIKE ? ESCAPE '\\'";terms.push('%'+q.toLowerCase().replace(/[\\%_]/g,'\\$&')+'%');}
        ...
        return json({items:items.results.map(unpack),total:count?.total??0,limit,offset,searchEngine:'d1-contains'});
      }
    ...
    console.error(JSON.stringify({event:'creezio-lite.api-error',requestId,type:error instanceof Error?error.name:'UnknownError'}));
