...
export function defineApp(input: unknown): AppDefinition {
  ...
  for (const mod of app.modules) {
    if (!idPattern.test(mod.id) || ids.has(mod.id) || ['overview','files','team','audit','settings'].includes(mod.id)) throw new Error('Identifiant de module invalide, réservé ou dupliqué.');
    ids.add(mod.id);
