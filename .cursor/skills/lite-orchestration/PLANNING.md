...
- **Revues** : reprise toujours sur le même agent et la même sélection via `correction.requested`, jamais un agent créé pour reprendre ; `closed` rouvrable explicitement vers `delivered` (événement `reopened`, preuve terminale conservée, `closedAt` déplacé), aucun autre statut ne se rouvre, aucun statut nouveau. Les validations du pilotage sont des conditions sourcées, pas des missions.
