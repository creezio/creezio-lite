...
- `scripts/cursor-agents.mjs` — `preflight`, `launch`, `reconcile`, `status`, `followup`, `successor`, `accounts`. Sorties JSON sans secret ; codes 0 ok, 2 bloqué, 3 indisponible ou incertain, 4 usage. Le pool de comptes passe par l’adaptateur optionnel `scripts/cursor-account-pool.mjs` du kit (ou `CURSOR_ACCOUNT_POOL_MODULE`) ; sans lui, `CURSOR_API_KEY` reste la seule voie.
