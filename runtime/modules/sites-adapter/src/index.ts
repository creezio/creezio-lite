...
export async function handleNativeApi(request:Request,context:ApiContext):Promise<Response|null> {
  ...
  try{
    ...
    const kernel=createApiKernel({brandId:context.app.id,appVersion:'0.3.0',authorizeModuleAccess:({permission})=>({allow:permissions(c,context.app).includes(permission),reason:'permission_denied'})});
