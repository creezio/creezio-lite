 ...
 * - Consommateurs : `subscribeDataChanged` / hook UI `useLiteResource`
 *   (rafraîchit la page ouverte sans reload manuel).
 *
 * Header HTTP homonyme (EntitySpec / mounts) : `x-lite-data-changed`
...
export const LITE_DATA_CHANGED_EVENT = "lite:data-changed";
export const LITE_DATA_CHANGED_HEADER = "x-lite-data-changed";

export type LiteDataChangedDetail = {
...
export function emitDataChanged(detail: LiteDataChangedDetail): void {
  if (typeof window === "undefined") return;
  const resource = String(detail.resource || "").trim();
  if (!resource) return;
  window.dispatchEvent(
    new CustomEvent(LITE_DATA_CHANGED_EVENT, {
      detail: {
        resource,
        ids: detail.ids,
        source: detail.source,
        at: detail.at ?? Date.now(),
      } satisfies LiteDataChangedDetail,
    }),
  );
}

export function subscribeDataChanged(
  handler: (detail: LiteDataChangedDetail) => void,
  ...
  const onEvent = (e: Event) => {
    const detail = (e as CustomEvent<LiteDataChangedDetail>).detail;
    if (!detail?.resource) return;
    if (wanted && !wanted.has(detail.resource)) return;
    handler(detail);
  };
  window.addEventListener(LITE_DATA_CHANGED_EVENT, onEvent);
  return () => window.removeEventListener(LITE_DATA_CHANGED_EVENT, onEvent);
}

/** Parse le header `x-lite-data-changed` (CSV) → resources. */
...
/**
 * Alias owner / stem outil → resource UI (`useLiteResource`).
 ...
 * `x-lite-data-changed`, émet le bus client (source `api`).
 */
export function installLiteDataChangedFetch(): void {
  ...
  window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    try {
      const header = res.headers.get(LITE_DATA_CHANGED_HEADER);
