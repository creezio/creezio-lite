...
export type ShellUiBrand = {
  /** Ex. liteDesktop / certivanDesktop / fiduDesktop */
  ...
  productName: string;
  /** Attribut data-* analytics clics (défaut data-lite-aid). */
  aidAttr?: string;
...
const DEFAULT: ShellUiBrand = {
  desktopApiGlobal: "liteDesktop",
  publicHostSuffix: "creez.io",
  titlebarDragClass: "lite-titlebar-drag",
  titlebarNoDragClass: "lite-titlebar-no-drag",
  apiKeyPrefix: "lite_live_",
  productName: "Lite",
  aidAttr: "data-lite-aid",
};
...
export function configureShellUiBrand(next: Partial<ShellUiBrand>): void {
  ...
  // No-op si aucune clé ne change — LiteUiBoot appelle au render (marque
 ...
 * LiteUiBoot en useEffect (après le 1er render) : les composants qui
 * lisent la brand au render (page login…) doivent passer par le hook
 * useShellUiBrand (@lite/shell-ui/ui/kit) pour re-render.
 */
