"use client";
import { getShellDesktopApi } from "@lite/shell-ui";
...
type TabWorkspaceContextValue = {
   ...
   *   pour rafraîchir via `useLiteResource`).
 ...
 * surfaces montées HORS du TabWorkspaceProvider (ex. @lite/interactive-demo
...
function loadPersisted(): WorkspacePersistedState | null {
  ...
  try {
    const legacyKey=Object.keys(sessionStorage).find(key=>key.endsWith('-workspace-tabs-v3'));
    const raw = sessionStorage.getItem(WORKSPACE_STORAGE_KEY) ?? (legacyKey ? sessionStorage.getItem(legacyKey) : null);
    if (!raw) return null;
