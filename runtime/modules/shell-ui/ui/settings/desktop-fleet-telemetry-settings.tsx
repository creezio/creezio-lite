  ...
  async function onScope(id: FleetScopeId, value: boolean) {
    ...
    if (value && meta?.sensitive) {
      const ok = window.confirm(
        "Je comprends que le contenu des conversations sera visible par le support Lite.",
      );
      if (!ok) return;
    }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Partage optionnel avec Lite Support. Tout est désactivé par défaut.
        ...
        <label className="flex cursor-pointer items-start justify-between gap-4 rounded-md border px-3 py-3">
          <div className="space-y-1">
            <Label className="text-sm font-medium">
              Partager des données avec Lite Support
            </Label>
