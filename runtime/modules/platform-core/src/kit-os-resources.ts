/**
 * Ressources OS natives shippées par le kit (@lite/host-runtime),
 * pas par la marque. Toute app Lite résout Hermes/n8n via ce chemin
 * si `resourcesRoot` marque n’embarque pas de vendor.
 *
 * P1.c (0.20) : `resources/{vendor,scripts,bin}` a quitté electron-shell
 * pour que l’image serveur headless n’embarque plus `@lite/electron-shell`
...
function resolvePackageRoot(
  spec: "@lite/electron-shell" | "@lite/host-runtime",
): string | null {
  try {
...
export function electronShellPackageRoot(): string {
  ...
  const resolved = resolvePackageRoot("@lite/electron-shell");
  cachedShellRoot =
...
function hostRuntimePackageRoot(): string {
  ...
  const resolved = resolvePackageRoot("@lite/host-runtime");
  cachedHostRoot =
