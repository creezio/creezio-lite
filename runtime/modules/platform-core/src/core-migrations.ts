 ...
 * - auth utilisateurs kit → `AUTH_CORE_SQL` (`lite_users`, pas table `users` legacy)
...
function loadAuthSql(req: NodeRequire): string {
  const mod = req("@lite/auth") as AuthSqlMod;
  if (typeof mod.AUTH_CORE_SQL !== "string" || !mod.AUTH_CORE_SQL.trim()) {
    throw new Error("platformCoreMigrations: AUTH_CORE_SQL introuvable (@lite/auth)");
  }
  return mod.AUTH_CORE_SQL;
}

function loadHubSql(req: NodeRequire): HubSqlMod {
  const mod = req("@lite/product-hub") as HubSqlMod;
  for (const key of [
    ...
    if (typeof mod[key] !== "string" || !mod[key].trim()) {
      throw new Error(
        `platformCoreMigrations: ${key} introuvable (@lite/product-hub)`,
      );
    }
  }
  return mod;
...
/**
 * Migrations SQLite cœur Lite (auth + Product Hub).
