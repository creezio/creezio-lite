 ...
 * PAS de `node_modules`. Les `@lite/*` vivent dans `app.asar`.
 * `createRequire(cwd/package.json)` → `Cannot find module '@lite/auth'`
 * (Require stack : …\TempoFlow-Server\package.json) alors que l’asar les
 * embarque correctement.
 *
 * Ancrage : ce module (dans asar → remonte vers `node_modules/@lite/*`),
...
export function createAppRequire(): NodeRequire {
  ...
  for (const anchor of candidates) {
    try {
      ...
      if (canResolve(req, probe) || canResolve(req, "@lite/platform-core")) {
        return req;
      }
    } catch {
      /* try next */
