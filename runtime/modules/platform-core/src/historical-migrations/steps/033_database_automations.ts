...
/**
 * Module Database avancé (style Notion) — SoT SQL = `@lite/database` (R1).
...
function loadDatabaseCoreSql(): string {
  const mod = createAppRequire()("@lite/database") as {
    DATABASE_CORE_SQL?: string;
  };
  if (typeof mod.DATABASE_CORE_SQL !== "string" || !mod.DATABASE_CORE_SQL.trim()) {
    throw new Error(
      "platformHistoricalMigrations: DATABASE_CORE_SQL introuvable (@lite/database)",
    );
  }
