 ...
 *
 * Contrat d'environnement (préfixe `LITE_`, variante marque `${PREFIX}_`
 * résolue d'abord — cohérent avec l'existant) :
 *   LITE_CF_API_TOKEN     (requis) — account token scopé compte+zone
 *   LITE_CF_ACCOUNT_ID    (requis)
 *   LITE_CF_ZONE_ID       (requis)
 *   LITE_CF_ZONE_NAME     (optionnel — dérivé via GET /zones/{zone_id})
 *   LITE_CF_UNIVERSAL_SSL (optionnel — D2 : truthy → hostnames nested)
 *   LITE_DOMAIN           (optionnel — hostname complet custom ;
 *                             défaut `{slug}.{zoneName}`)
 *   LITE_TUNNEL_SLUG      (optionnel — sinon nom d'instance / brandId)
 *   LITE_TUNNEL_EXTRA_HOSTNAMES (optionnel — D1 multi-hostnames, virgules)
 *
...
function pickEnv(
  ...
  return (env[`LITE_${key}`] || "").trim();
}

/**
 * Résout le contrat CF depuis l'environnement. Ordre : `${prefix}_CF_*`
 * (marque) puis `LITE_CF_*` (générique kit). Retourne null si un requis
...
export function missingCfTunnelEnvKeys(
  ...
  for (const key of ["CF_API_TOKEN", "CF_ACCOUNT_ID", "CF_ZONE_ID"]) {
    if (!pickEnv(env, brandPrefix, key)) missing.push(`LITE_${key}`);
  }
  return missing;
}
...
/** Nom de la zone — env `LITE_CF_ZONE_NAME` ou `GET /zones/{zone_id}`. */
...
export type CfTunnelEnsureOpts = {
  slug: string;
  /** Hostname complet custom (LITE_DOMAIN) — défaut `{slug}.{zone}`. */
  domain?: string;
...
export async function ensureCfTunnel(
  ...
  if (!tunnelId) {
    const namePrefix = opts.tunnelNamePrefix || "lite-server-";
...
export type CfAgentTunnelEnsureOpts = {
  /** Slug d'où dérive le hostname agent (nom CF `lite-agent-<slug>`). */
  slug: string;
...
export async function ensureCfAgentTunnelDns(
  ...
  return ensureCfCnameRecord(env, {
    ...
    comment: `Lite agent ${opts.slug || opts.hostname}`,
  });
}
