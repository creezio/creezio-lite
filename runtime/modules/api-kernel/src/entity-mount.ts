 ...
 *                         `?ids=`, archives) ou avec `LITE_ALLOW_NO_MEILI=1`
...
/** Header HTTP → bus client `lite:data-changed` (voir @lite/shell-ui). */
...
export type EntitySpec = {
  ...
  table: string;
  /**
   * Resource pour le bus UI `lite:data-changed` (header HTTP).
   * Défaut = `table`. Les pages liste déclarent la même resource via
   * `useLiteResource(resource)`.
   */
  resource?: string;
  /**
...
export function createEntityApiMount(spec: EntitySpec): ApiMount {
  ...
  function withDataChanged(res: ApiResponse): ApiResponse {
    ...
    return {
      ...
      headers: {
        ...(res.headers || {}),
        [LITE_DATA_CHANGED_HEADER]: resource,
      },
    };
  }
  ...
  async function handleList(ctx: EntityHookContext): Promise<ApiResponse> {
    ...
    if (binding && archived !== "1") {
      ...
      // LIKE SQL. Échappatoire dev/tests hors-browse : LITE_ALLOW_NO_MEILI=1.
      if (process.env.LITE_ALLOW_NO_MEILI === "1") {
        return listFromSql(ctx, {
          engine: "sql",
