 ...
 * Utilisé par `@lite/factory` (Phase D) — jamais pour écraser
...
export type AppManifestSpec = {
  ...
  sandbox?: boolean;
  /** Racine app locale (défaut `/opt/docker/lite/apps/<brandId>`). */
  defaultAppRoot?: string;
  /** Préfixe reverse-DNS appId (défaut `io.lite`). */
  appIdPrefix?: string;
  /** Hôte remote-build (défaut parc Lite). */
   ...
   * (`n8n-{slug}.{zone}`). Défaut nested. Override : LITE_TUNNEL_FLAT_HOSTS.
   */
...
export function defaultFeedToken(brandId: string): string {
  ...
  const digest = nsisGuidFromAppId(`lite-feed:${brandId}`).replace(
    /-/g,
    "",
  );
...
export function createAppManifest(spec: AppManifestSpec): AppManifest {
  ...
  const appIdPrefix = (spec.appIdPrefix || "io.lite").replace(/\.$/, "");
  ...
  const defaultAppRoot =
    spec.defaultAppRoot || `/opt/docker/lite/apps/${brandId}`;
  ...
  const publish: BrandPublishInfra = {
    ...
    remoteBinSrc: `/opt/docker/lite/apps/${brandId}`,
