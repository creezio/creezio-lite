 ...
 * ```ts
 * import { buildElectronBuilderConfig, demobrandManifest } from "@lite/brand-config";
...
/**
 * Plancher @lite/* toujours emballé (boot main Electron).
 * La liste effective = plancher ∪ deps `@lite/*` du package.json marque
 * ∪ clôture transitive npm — voir `collectLiteRuntimePackages()`.
 *
 * Les apps marques consomment `@lite/*` en packages npm (GitHub
 * Packages). electron-builder exclut `node_modules/**` → FileSet par
 * package résolu (walk-up : `node_modules/…` ou `../node_modules/…` en
 * workspaces) vers `node_modules/@lite/<pkg>` dans l'asar.
 *
 * Ne plus maintenir une allowlist partielle seule : tout oubli = MODULE_NOT_FOUND
 * packagé. Les deps npm « vraies » (hono, zod…) → `LITE_ASAR_NPM_RUNTIME_PACKAGES`.
 */
export const LITE_ASAR_RUNTIME_PACKAGES = [
  "brand-config",
  "platform-core",
  "product-hub",
  "shell",
...
/**
 * Seeds npm runtime du main Electron (hors @lite/*).
 *
 ...
 */
export const LITE_ASAR_NPM_RUNTIME_PACKAGES = [
...
export const LITE_ASAR_NPM_INSTALL_ONLY = [
  "prebuild-install",
  "node-abi",
  "napi-build-utils",
  "simple-get",
...
export const LITE_ASAR_UNPACK_NATIVE = [
  "**/*.node",
 ...
 * Surcharge : env `LITE_WIN_BIN_STAGE`.
 */
export const DEFAULT_WIN_BIN_STAGE = ".lite/win-bin-stage";
...
const LITE_ASAR_WITH_UI = new Set([
  "shell-ui",
  "os-ui",
  "product-hub",
  "auth",
...
function resolveAppRoot(appRoot?: string): string {
  return path.resolve(
    appRoot || process.env.LITE_APP_ROOT || process.cwd(),
  );
}
 ...
 *          `../node_modules/@lite/hono`), null si absent.
 */
...
/**
 * Clôture @lite/* runtime pour l'asar :
 * plancher `LITE_ASAR_RUNTIME_PACKAGES`
 * ∪ `@lite/*` du package.json marque (hors tooling)
 * ∪ deps `@lite/*` transitives des package.json installés (node_modules,
 * walk-up workspaces). Ne retourne que les packages réellement résolus.
 */
export function collectLiteRuntimePackages(appRoot?: string): string[] {
  const root = resolveAppRoot(appRoot);
  const tooling = new Set<string>(LITE_ASAR_TOOLING_ONLY);
  const seeds = new Set<string>(LITE_ASAR_RUNTIME_PACKAGES);

  for (const dep of readJsonDeps(path.join(root, "package.json"))) {
    if (!dep.startsWith("@lite/")) continue;
    const short = dep.slice("@lite/".length);
  ...
  while (queue.length) {
    ...
    const pkgJson = resolveInstalledPkgJson(root, `@lite/${name}`);
    if (!pkgJson) continue;
    seen.add(name);
  ...
  if (!seen.size) return [...LITE_ASAR_RUNTIME_PACKAGES];
  return [...seen].sort();
}

function liteAsarFileSets(appRoot?: string): JsonRecord[] {
  const root = resolveAppRoot(appRoot);
  const names = collectLiteRuntimePackages(appRoot);
  return names.map((name) => {
    const filter = [
      ...
      "dist-cjs/**/*",
      "!resources/bin/**",
      ...(LITE_ASAR_WITH_UI.has(name) ? ["ui/**/*"] : []),
    ];
    return {
      from:
        resolveInstalledRel(root, `@lite/${name}`) ??
        `node_modules/@lite/${name}`,
      to: `node_modules/@lite/${name}`,
      filter,
    };
  });
}
...
export function collectNpmRuntimePackages(appRoot?: string): string[] {
  const root = resolveAppRoot(appRoot);
  const installOnly = new Set<string>(LITE_ASAR_NPM_INSTALL_ONLY);
  const seen = new Set<string>();
  const queue: string[] = [...LITE_ASAR_NPM_RUNTIME_PACKAGES];

  // Seeds supplémentaires depuis dependencies npm « vraies » de la marque
  // (hors @lite/* et tooling electron-builder).
  const brandSkip = new Set([
    ...LITE_ASAR_NPM_INSTALL_ONLY,
    "electron",
    "electron-builder",
    "electron-updater",
    "typescript",
  ...
  while (queue.length) {
    ...
    seen.add(name);
    for (const dep of readJsonDeps(pkgJson)) {
      if (!seen.has(dep) && !installOnly.has(dep) && !dep.startsWith("@lite/")) {
        queue.push(dep);
      }
    }
  }
...
function npmRuntimeAsarFileSets(appRoot?: string): JsonRecord[] {
  ...
  const list = names.length ? names : [...LITE_ASAR_NPM_RUNTIME_PACKAGES];
...
function ensureAsarUnpackNative(base: JsonRecord): void {
  const prev = Array.isArray(base.asarUnpack)
    ...
    : [];
  const next = [...prev];
  for (const pat of LITE_ASAR_UNPACK_NATIVE) {
...
function stripLegacyNpmRuntimeGlobs(files: unknown[]): unknown[] {
  return files.filter((entry) => {
    ...
    return !(LITE_ASAR_NPM_RUNTIME_PACKAGES as readonly string[]).includes(
      pkg,
    );
  });
}
...
/**
 * Ré-inclut les packages runtime @lite/* + deps npm (clôture) dans l'asar
 ...
 */
function ensureLiteVendorInAsar(
  base: JsonRecord,
  ...
  const next = [...files];
  for (const set of liteAsarFileSets(appRoot)) {
...
function resolveWinBinStage(options: BuildBuilderConfigOptions): string {
  return (
    options.winBinStage ||
    process.env.LITE_WIN_BIN_STAGE ||
...
export function buildElectronBuilderConfig(
  ...
  ensureLiteVendorInAsar(
    base,
    options.packLiteVendor,
    options.appRoot || process.env.LITE_APP_ROOT,
  );
  ...
  if (!base.afterPack) {
    // Package npm @lite/desktop-tooling résolu walk-up (workspaces).
    const appRoot = options.appRoot || process.env.LITE_APP_ROOT || "";
    const fallback = "node_modules/@lite/desktop-tooling/scripts/after-pack.cjs";
    let resolved: string | null = null;
    if (appRoot) {
      const pkgRel = resolveInstalledRel(appRoot, "@lite/desktop-tooling");
      if (pkgRel) {
  ...
  ensureKitOsVendorExtraResources(base, {
    ...
    appRoot: options.appRoot || process.env.LITE_APP_ROOT,
  });
 ...
 * npm `@lite/host-runtime/resources/vendor` — jamais depuis la marque.
...
function ensureKitOsVendorExtraResources(
  ...
  const hostRel = opts.appRoot
    ? resolveInstalledRel(opts.appRoot, "@lite/host-runtime")
    : null;
  const hostFrom = hostRel ?? "node_modules/@lite/host-runtime";
