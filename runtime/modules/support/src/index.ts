/**
 * @lite/support — support natif OS côté serveur marque.
 *
 ...
 * push serveur → admin).
 *
 * Montage natif : `create-brand-kernel.ts` (@lite/app-runtime) enregistre
 * `platform-support` comme tasks/mails → HTTP
 * `/api/v1/platform/platform-support/*`, données en core.db.
 */

import type { ApiMount } from "@lite/api-kernel";

/* ------------------------------------------------------------ schéma */

export const SUPPORT_CORE_SQL = `-- Support natif OS (@lite/support)
