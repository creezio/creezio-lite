...
const DEFAULT_IDENTITY: AssistantBrandIdentity = {
  productName: "Lite",
  uiStorageKey: "lite-assistant-ui",
  modeStorageKey: "lite-assistant-preferred-mode",
  desktopApiGlobal: "liteDesktop",
  globalStorePrefix: "__lite",
};
...
export function requireAssistantBrand(): AssistantBrandConfig {
  if (!config) {
    throw new Error(
      "@lite/assistant: configureAssistantBrand() requis avant usage runtime",
    );
  }
  return config;
}
...
export function requireAssistantDb(): AssistantDbAccess {
  ...
  if (!db) {
    throw new Error(
      "@lite/assistant: configureAssistantBrand({ db }) requis pour SQL tools",
    );
  }
  return db;
}
