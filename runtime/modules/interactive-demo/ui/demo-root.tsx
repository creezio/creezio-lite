 ...
 * `lite-interactive-demo` pour un déclenchement programmatique.
 *
...
import type { DemoScenario } from "@lite/interactive-demo";
import { scenarioMatchesRole } from "@lite/interactive-demo";
import { registerSidebarActionsProvider } from "@lite/shell-ui/ui";
...
export const INTERACTIVE_DEMO_EVENT = "lite-interactive-demo";
...
export type InteractiveDemoRootProps = {
   ...
   * (registre @lite/shell-ui — visible uniquement dans le workspace
  ...
  return (
    <div data-lite-demo-ui="1">
      {menuOpen && visible.length > 1 ? (
        <div className="lite-demo-launcher-menu">
          {visible.map((s) => (
            <button
              key={s.id}
              type="button"
              className="lite-demo-launcher-item"
              onClick={() => start(s.id)}
            >
              <p className="lite-demo-launcher-item-title">{s.title}</p>
              {s.description ? (
                <p className="lite-demo-launcher-item-desc">{s.description}</p>
              ) : null}
            </button>
          ))}
        </div>
