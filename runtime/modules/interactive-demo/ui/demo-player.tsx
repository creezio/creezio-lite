 ...
 * marque. Styles : `@lite/interactive-demo/ui/interactive-demo.css`.
 */

import { useCallback, useEffect, useRef, useState } from "react";
import { getWorkspaceTabNavigate } from "@lite/shell-ui/ui";
import type {
  DemoScenario,
  DemoStep,
  DemoTarget,
} from "@lite/interactive-demo";
...
export function DemoPlayer({
  ...
  return (
    <div data-lite-demo-ui="1">
      {veil ? <div className="lite-demo-veil" /> : null}
      {spotlight ? (
        <div
          className="lite-demo-spotlight"
          style={{
      ...
      {card ? (
        <div
          className={
            card.centered
              ? "lite-demo-card lite-demo-card--centered"
              : "lite-demo-card"
          }
          ...
          aria-live="polite"
        >
          <p className="lite-demo-card-kicker">{card.kicker}</p>
          {card.title ? (
            <h3 className="lite-demo-card-title">{card.title}</h3>
          ) : null}
          {card.body ? (
            <p className="lite-demo-card-body">{card.body}</p>
          ) : null}
          {card.showNext ? (
            <div className="lite-demo-card-actions">
              <button
                type="button"
                className="lite-demo-btn lite-demo-btn--ghost"
                onClick={exit}
              >
                Quitter
              </button>
              <button
                type="button"
                className="lite-demo-btn lite-demo-btn--primary"
                onClick={advance}
              >
      ...
      {note ? <div className="lite-demo-note">{note}</div> : null}

      <div className="lite-demo-controls">
        <span className="lite-demo-controls-label">{scenario.title}</span>
        <span className="lite-demo-progress">
          <span
            className="lite-demo-progress-fill"
            style={{ width: `${Math.round(((stepIndex + 1) / total) * 100)}%` }}
          />
        </span>
        <span className="lite-demo-controls-count">
          {stepIndex + 1}/{total}
        </span>
        <button
          type="button"
