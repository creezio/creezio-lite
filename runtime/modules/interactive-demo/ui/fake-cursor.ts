 ...
 * curseur de l'assistant (`@lite/assistant`) : la démo est un scénario
 * scripté, pas une action LLM.
 *
 * Styles : classes `.lite-demo-*` (feuille `interactive-demo.css`).
 */

const CURSOR_ID = "lite-demo-cursor";
const HIDE_DELAY_MS = 2600;

const CURSOR_SVG = `
<svg width="26" height="26" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  <path d="M4 2.5 L4 18.5 L8.2 14.8 L11 21 L13.6 19.8 L10.9 13.8 L16.5 13.3 Z"
    fill="var(--lite-demo-accent, #7c3aed)" stroke="white" stroke-width="1.4" stroke-linejoin="round"/>
</svg>`;
...
class DemoCursor {
  ...
  private ensure(): HTMLDivElement {
    ...
    wrap.id = CURSOR_ID;
    wrap.className = "lite-demo-cursor";
    wrap.setAttribute("data-lite-demo-ui", "1");
    wrap.innerHTML = CURSOR_SVG;

    const badge = document.createElement("div");
    badge.textContent = "Démo";
    badge.className = "lite-demo-cursor-badge";
  ...
  hide() {
    ...
    if (this.el) {
      ...
      /* Terminer / Quitter : retirer le nœud. hide() historique laissait
         #lite-demo-cursor (data-lite-demo-ui) → E2E « overlay reste ». */
      this.el.remove();
      this.el = null;
      this.badge = null;
    }
  ...
  clickEffect(): Promise<void> {
    ...
    ripple.className = "lite-demo-click-ripple";
    ripple.setAttribute("data-lite-demo-ui", "1");
