...
import { Button, Input, Textarea } from "@lite/shell-ui/ui/kit";
...
export type MailComposerProps = {
  apiBase: string;
  senders:{id:string;label:string;from:string}[];
  open: boolean;
  onClose: () => void;
  /** Après envoi ou sauvegarde brouillon réussis. */
  onDone: (kind: "sent" | "draft" | "outbox") => void;
...
function loadTiptap(): Promise<TiptapModules | null> { return Promise.resolve(null); }
...
export function MailComposer(props: MailComposerProps) {
  ...
  useEffect(() => {
    ...
    setIntegrationId(initial?.integrationId || props.senders[0]?.id || "");
  ...
  function payloadBase() {
    return {
      version,integrationId:integrationId || undefined,idempotencyKey,
      to,
    ...
    try {
      if (draftId) {
        ...
        if (!r.ok) {
          ...
          throw new Error((typeof j.error === "string" ? j.error : j.error?.message) || "sauvegarde échouée");
        }
        const saved=await r.json();setVersion(saved.mail?.version);
        setAttachments([]);
      } else {
        ...
        const j = await r.json().catch(() => ({}));
        if (!r.ok) throw new Error((typeof j.error === "string" ? j.error : j.error?.message) || "sauvegarde échouée");
        setDraftId(j.mail?.id || null);setVersion(j.mail?.version);
        setAttachments([]);
      }
      props.onDone("draft");
    } catch (e) {
  ...
  async function send() {
    ...
    try {
      let r: Response;
      if (draftId) {
        const saved = await fetch(`${props.apiBase}/drafts/${draftId}`, {
          method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(payloadBase()),
        });
        const snapshot=await saved.json();if(!saved.ok)throw new Error(snapshot.error?.message || "Sauvegarde impossible.");
        setVersion(snapshot.mail?.version);setAttachments([]);
      ...
      const j = await r.json().catch(() => ({}));
      if (!r.ok) {if(j.mail?.id)props.onDone("outbox");throw new Error((typeof j.error === "string" ? j.error : j.error?.message) || "envoi échoué");}
      props.onDone("sent");
      props.onClose();
    } catch (e) {
  ...
  return (
    <div role="dialog" aria-modal="true" aria-label="Rédiger un e-mail" className="fixed inset-0 z-50 flex items-end justify-center bg-black/30 p-4 sm:items-center">
      <div className="flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-[#e6e0d4] bg-white shadow-xl">
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-2.5 overflow-y-auto px-4 py-3">
            <label className="flex items-center gap-3 text-sm">De<select aria-label="Compte expéditeur" value={integrationId} onChange={e=>setIntegrationId(e.target.value)} className="min-w-0 flex-1 rounded-md border bg-white px-2 py-1.5"><option value="">Choisir une connexion</option>{props.senders.map(sender=><option key={sender.id} value={sender.id}>{sender.label}</option>)}</select></label>
