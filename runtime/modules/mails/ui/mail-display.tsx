...
function SandboxedHtml(props: { html: string; title: string }) {
  const srcDoc = useMemo(() => {
    ...
    return `<!doctype html><html><head><meta charset="utf-8"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; img-src data:; style-src 'unsafe-inline';"><base target="_blank"><style>
...
export function MailDisplay(props: MailDisplayProps) {
  ...
  const showJournal = Boolean(
    detail &&
      ["queued", "sending", "failed", "failed_permanent", "bounced", "delivery_unknown", "partial", "delivery_unknown", "partial"].includes(
        detail.status,
      ),
  );
  ...
  async function patch(body: Record<string, unknown>) {
    ...
    try {
      const r = await fetch(`${apiBase}/${detail.id}`, {
        ...
        body: JSON.stringify({...body,version:detail.version}),
      });
      if (r.ok) {setError("");props.onChanged();} else {const body=await r.json();setError(body.error?.message || "Modification impossible.");}
    } finally {
      props.setBusy(false);
    }
  }
  ...
  async function remove() {
    ...
    props.setBusy(true);
    try {
      const r = await fetch(`${apiBase}/${detail.id}?version=${detail.version}`, { method: "DELETE" });
      if (r.ok) {setError("");props.onChanged();} else {const body=await r.json();setError(body.error?.message || "Modification impossible.");}
    } finally {
      props.setBusy(false);
    }
  ...
  return (
    <TooltipProvider delayDuration={300}>
      <div className="flex h-full min-h-0 flex-col bg-[#fcfbf8]">
        ...
        {error?<p role="alert" className="px-5 py-2 text-sm text-red-700">{error}</p>:null}
        {detail.last_error?<p role="status" className="px-5 py-2 text-sm text-amber-700">{detail.last_error}</p>:null}
        {detail.status==="failed_permanent" && props.canWrite?<Button className="mx-5 my-2" disabled={props.busy || !props.canWrite} onClick={async()=>{props.setBusy(true);try{const r=await fetch(`${apiBase}/${detail.id}/retry`,{method:"POST"});if(!r.ok){const j=await r.json();setError(j.error?.message || "Envoi refusé.");}props.onChanged();}finally{props.setBusy(false);}}}>Réessayer l’envoi</Button>:null}
