...
async function remote(url:string,key:string,body?:unknown,extra:Record<string,string>={}){
  let response:Response;try{response=await fetch(url,{method:body===undefined?'GET':'POST',redirect:'error',signal:AbortSignal.timeout(25000),headers:{Authorization:`Bearer ${key}`,...(body===undefined?{}:{'content-type':'application/json'}),...extra},...(body===undefined?{}:{body:JSON.stringify(body)})});}catch{fail(502,'mail_delivery_unknown','Le service n’a pas confirmé le résultat. Vérifiez chez le fournisseur avant de renvoyer le message.');}
