...
export async function accessRoute(request:Request,c:ApiContext,org:Workspace,operations:Operation[]):Promise<Response|null>{
  ...
  if(path==='access/catalog'&&request.method==='GET'){
    ...
    return json({groups:groups.map(g=>({...g,policyVersion:versions.results.find(v=>v.group_id===g.id)?.version??0})),members:members.results,policies:policies.results,operations:operations.map(op=>({...op,locked:Boolean(op.essential),defaultGroups:op.roles.map(r=>`role:${r}`)}))});
  }
