...
export function validateSchema(schema:JsonSchema,value:unknown,path='arguments'):void {
  if(schema.anyOf){for(const option of schema.anyOf){try{validateSchema(option,value,path);return;}catch{}}invalid(path,' : valeur invalide.');}
  ...
  if(schema.type==='object'){
    ...
    const v=value as Record<string,unknown>;if((schema.required??[]).some((key:string)=>v[key]===undefined))invalid(path,' : champ obligatoire absent.');
    for(const [key,item] of Object.entries(v)){const child=schema.properties?.[key];if(!child&&schema.additionalProperties===false)invalid(path+'.'+key,' : champ inconnu.');if(child)validateSchema(child,item,path+'.'+key);}
  }
  if(schema.type==='array'){
    if(!Array.isArray(value)||(schema.maxItems!==undefined&&value.length>schema.maxItems))invalid(path,' : liste invalide.');
  ...
  if(schema.type==='string'){
    ...
    if(schema.format==='email'&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value))invalid(path,' : e-mail invalide.');
  ...
  if(schema.type==='number'||schema.type==='integer')if(typeof value!=='number'||!Number.isFinite(value)||(schema.type==='integer'&&!Number.isInteger(value))||(schema.minimum!==undefined&&value<schema.minimum)||(schema.maximum!==undefined&&value>schema.maximum))invalid(path,' : nombre invalide.');
  if(schema.type==='boolean'&&typeof value!=='boolean')invalid(path,' : booléen requis.');
}
