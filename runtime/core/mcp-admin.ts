...
export async function mcpAdminRoute(request:Request,c:ApiContext,org:Workspace,operations:Operation[]):Promise<Response|null>{
  ...
  if(path==='admin/mcp/diagnostics'||path==='admin/mcp/diagnostics/export'){
    ...
    const response=json({healthy:checks.every(c=>c.ok),checks,version:'0.15.2',operations:operations.length,tools:bindings.length});if(path.endsWith('/export'))response.headers.set('Content-Disposition','attachment; filename="lite-mcp-diagnostic.json"');return response;
  }
