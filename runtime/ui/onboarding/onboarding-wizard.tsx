...
export function OnboardingWizard({
  ...
  const persistStep = useCallback(
    async (next: number) => {
      ...
      try { await transport.persistStep(next); return true; }
      catch(error) { setError(error instanceof Error?error.message:'Enregistrement impossible.'); return false; }
  ...
  const goTo = useCallback(
    async (next: number) => {
      ...
      if(!await persistStep(clamped))return;
      setEntry("start");
      setStep(clamped);
  ...
  const back = useCallback(async () => {
    ...
    if(!await persistStep(prevStep))return;
    setEntry("end");
    setStep(prevStep);
    window.scrollTo({ top: 0 });
  }, [persistStep, step]);
