...
export function RecordForm({module,record,api,onSaved,onCancel,draft,onDraftChange}:{module:Module;record:RecordData|null;api:Api;onSaved:()=>void;onCancel:()=>void;draft?:Record<string,unknown>;onDraftChange?:(value:Record<string,unknown>)=>void}) {
  const formId=useId();
  const [localValues,setLocalValues]=useState<Record<string,unknown>>(()=>record?.data??Object.fromEntries(module.fields.map(f=>[f.key,f.type==='boolean'?false:''])));
...
export function moduleRecordColumns(module:Module):ColumnDef<RecordData>[] {
  ...
  return visible.map(field=>({id:field.key,accessorFn:row=>formatFieldValue(field,row.data[field.key]),header:field.label,cell:({row})=>{
