...
export function ModuleView({module,api,role,revision,onMutation}:{module:Module;api:Api;role:Role;revision:number;onMutation:()=>void}) {
  ...
  const {data,error,loading}=useLoad<{items:RecordData[];total:number}>(()=>api(`modules/${module.id}/records?${params}`),[api,module.id,search,offset,filter,revision],[api,module.id,search,offset,filter]);
