'use client';
import {editableFields} from '../core/entity-fields.ts';
...
import {fieldInputValue,parseFieldInput,formatFieldValue} from './field-value';
...
function RecordReferenceInput({field,api,value,onChange,id}:{field:Field;api:Api;value:string;onChange:(value:string)=>void;id:string}) {
  const ref=field.reference!;
  const load=useCallback(async(q:string)=>{
    const result=await api<{items:RecordData[]}>('modules/'+ref.moduleId+'/records?limit=100&q='+encodeURIComponent(q));
    return result.items.map(record=>({id:record.id,label:String(record.data[ref.labelField??'name']??record.data.title??record.id)}));
  },[api,ref.moduleId,ref.labelField]);
  return <ReferencePicker sourceKey={ref.moduleId+':'+field.key} id={id} load={load} value={value} onChange={onChange} label={field.label} required={field.required} multiple={ref.multiple}/>;
}
export function RecordForm({module,record,api,onSaved,onCancel,draft,onDraftChange}:{module:Module;record:RecordData|null;api:Api;onSaved:()=>void;onCancel:()=>void;draft?:Record<string,unknown>;onDraftChange?:(value:Record<string,unknown>)=>void}) {
  const formId=useId();
  const [localValues,setLocalValues]=useState<Record<string,unknown>>(()=>record?.data??Object.fromEntries(editableFields(module).map(f=>[f.key,f.type==='boolean'?false:''])));
  const values=draft??localValues;
  const setValues=(next:Record<string,unknown>)=>{setLocalValues(next);onDraftChange?.(next);};
  const [busy,setBusy]=useState(false),[error,setError]=useState('');
  async function submit(event:React.FormEvent){event.preventDefault();setBusy(true);setError('');try{const data=Object.fromEntries(editableFields(module).map(f=>[f.key,f.type==='number'&&values[f.key]!==''&&values[f.key]!==null?Number(values[f.key]):values[f.key]]));await api(`modules/${module.id}/records${record?'/'+record.id:''}`,{method:record?'PATCH':'POST',body:JSON.stringify({data,...(record?{version:record.version}:{})})});toast.success(record?'Modifications enregistrées':'Document créé');onSaved();}catch(e){setError((e as Error).message);}finally{setBusy(false);}}
  return <form onSubmit={submit} className="record-form"><div className="form-fields">{editableFields(module).map(field=><div className={field.type==='textarea'?'field field-wide':'field'} key={field.key}><Label htmlFor={`${formId}-${field.key}`}>{field.label}{field.required?' *':''}</Label>{field.reference?<RecordReferenceInput field={field} api={api} id={formId+'-'+field.key} value={String(values[field.key]??'')} onChange={value=>setValues({...values,[field.key]:value})}/>:field.type==='select'?<Select value={String(values[field.key]??'')} onValueChange={v=>setValues({...values,[field.key]:v})}><SelectTrigger id={`${formId}-${field.key}`} aria-required={field.required}><SelectValue placeholder="Choisir…"/></SelectTrigger><SelectContent>{field.options?.map(option=><SelectItem key={option} value={option}>{option}</SelectItem>)}</SelectContent></Select>:field.type==='boolean'?<Checkbox id={`${formId}-${field.key}`} checked={values[field.key]===true} onCheckedChange={v=>setValues({...values,[field.key]:v===true})}/>:field.type==='textarea'?<Textarea id={`${formId}-${field.key}`} value={fieldInputValue(field,values[field.key])} onChange={e=>setValues({...values,[field.key]:e.target.value})} required={field.required} maxLength={field.maxLength??5000} rows={4}/>:<Input id={`${formId}-${field.key}`} type={['number','email','date'].includes(field.type)?field.type:'text'} value={fieldInputValue(field,values[field.key])} onChange={e=>setValues({...values,[field.key]:parseFieldInput(field,e.target.value)})} required={field.required} min={field.min===undefined?undefined:field.min/(field.scale??1)} max={field.max===undefined?undefined:field.max/(field.scale??1)} maxLength={field.maxLength??300} step={field.type==='number'?(field.integer?1/(field.scale??1):'any'):undefined}/>}</div>)}</div><State error={error}/><div className="form-actions"><Button type="button" variant="outline" onClick={onCancel} disabled={busy}>Annuler</Button><Button type="submit" disabled={busy}>{busy?'Enregistrement…':'Enregistrer'}</Button></div></form>;
}
/** Record links use the shared workspace navigation; modifier clicks keep its tab policy. */
...
export function ModuleView({module,api,role,revision,onMutation}:{module:Module;api:Api;role:Role;revision:number;onMutation:()=>void}) {
  const router=useRouter(),selectedId=useSearchParams().get('record'),active=usePaneActive();
  ...
  const editRecord=(record:RecordData|null)=>{setDraft(record?.data??Object.fromEntries(module.fields.map(f=>[f.key,f.type==='boolean'?false:''])));setEditing(record);};
  ...
  const params=new URLSearchParams({q:search,offset:String(offset),limit:'30'});if(sort){params.set('sort',sort.id);params.set('direction',sort.desc?'desc':'asc');}if(filter!=='all'&&statusField){params.set('field',statusField.key);params.set('value',filter);}
  const {data,error,loading}=useLoad<{items:RecordData[];total:number}>(()=>api(`modules/${module.id}/records?${params}`),[api,module.id,search,offset,filter,sort?.id,sort?.desc,revision],[api,module.id,search,offset,filter,sort?.id,sort?.desc]);
  const columns=moduleRecordColumns(module);
  if(canWrite)columns.push({id:'actions',header:'Actions',enableSorting:false,cell:({row})=><div className="flex gap-1"><Button variant="ghost" size="icon" aria-label={`Modifier ${row.original.data[module.titleField]}`} onClick={()=>editRecord(row.original)}><Pencil size={15}/></Button><Button variant="ghost" size="icon" aria-label={`Archiver ${row.original.data[module.titleField]}`} onClick={()=>setArchiving(row.original)}><Archive size={15}/></Button></div>});
  ...
  return <section className="workspace-section space-y-5"><EntityHeader title={module.name} description={module.description} actions={canWrite?<Button onClick={()=>editRecord(null)}><Plus size={16}/>Nouveau {module.singular.toLowerCase()}</Button>:undefined}/>
    <State error={error}/>{loading?<p className="text-sm text-slate-500" role="status">Chargement…</p>:null}
    {selectedId?<RecordDetail module={module} api={api} id={selectedId} revision={revision} onClose={()=>router.replace(`/${module.id}`)} onEdit={canWrite?record=>{router.replace(`/${module.id}`);editRecord(record);}:undefined}/>:null}
    <DataTable columns={columns} data={data?.items??[]} remoteSorting={{value:sorting,onChange:value=>{setSorting(value);setOffset(0);}}} searchPlaceholder={`Rechercher dans ${module.name}…`} searchValue={query} onSearchValueChange={setQuery} remotePage={{index:offset/30,size:30,total:data?.total??0,onChange:index=>setOffset(index*30)}} filterSlot={statusField?<Select value={filter} onValueChange={v=>{setFilter(v);setOffset(0);}}><SelectTrigger className="w-48" aria-label={`Filtrer par ${statusField.label}`}><SelectValue/></SelectTrigger><SelectContent><SelectItem value="all">Tous les statuts</SelectItem>{statusField.options?.map(v=><SelectItem value={v} key={v}>{v}</SelectItem>)}</SelectContent></Select>:undefined}/>
    <Dialog open={active&&editing!==undefined} onOpenChange={open=>{if(!open&&active)setEditing(undefined);}}><DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle>{editing?'Modifier':'Créer'} {module.singular.toLowerCase()}</DialogTitle><DialogDescription>{module.description}</DialogDescription></DialogHeader>{editing!==undefined?<RecordForm key={editing?.id??'new'} module={module} record={editing} api={api} draft={draft} onDraftChange={setDraft} onCancel={()=>setEditing(undefined)} onSaved={()=>{setEditing(undefined);onMutation();}}/>:null}</DialogContent></Dialog>
    <Dialog open={active&&Boolean(archiving)} onOpenChange={open=>{if(!open&&active&&!busy)setArchiving(null);}}><DialogContent><DialogHeader><DialogTitle>Archiver ce document ?</DialogTitle><DialogDescription>Il sera retiré de la liste. Les données et l’historique seront conservés.</DialogDescription></DialogHeader><div className="flex justify-end gap-2"><Button variant="outline" disabled={busy} onClick={()=>setArchiving(null)}>Annuler</Button><Button disabled={busy} onClick={()=>void archive()}>Archiver</Button></div></DialogContent></Dialog>
  </section>;
}
