...
- Constat non généralisé : un lancement réel source SHA a reçu `400 validation_error` (catalogue visible, puis GET 404 `not_created`) ; un mono-repo cible SHA a échoué puis une cible branche a été acceptée après réconciliation. Docs officielles : SHA autorisé. Limite « SHA hors default/PR » **non prouvée** pour une source. Pas de remplacement SHA→branche, pas d’omission de source, pas de sonde nouvelle, pas de nouvelle référence autorisée par le kit. Échec visible ≠ crédits épuisés. Une prochaine réception utile (orchestrateur, hors ce commit) peut attacher un SHA **déjà présent sur la branche default** d’un dépôt source déjà approuvé au catalogue, pour distinguer cette contrainte d’un échec d’attachement. Capacité non publiée tant qu’un appel accepté utile et un checkout observé n’ont pas été obtenus.
