...
- Constat non généralisé : un lancement réel source SHA a reçu `400 validation_error` (catalogue visible, puis GET 404 `not_created`) ; un mono-repo cible SHA a échoué puis une cible branche a été acceptée après réconciliation. Docs officielles : SHA autorisé. Limite « SHA hors default/PR » **non prouvée** pour une source. Pas de remplacement SHA→branche, pas d’omission de source, pas de sonde nouvelle. Échec visible ≠ crédits épuisés. Capacité non publiée tant qu’un appel accepté utile et un checkout observé n’ont pas été obtenus.
- `SKILL.md` : boucle active d’orchestration (rester dans le tour, outils existants, polling progressif borné ; `RUNNING`/CI pending n’est pas une fin). `status --follow` émet au changement et continue tant que non terminal. Frontières kit/app. Renvoi court `CONTRACT.md` §10. Pas de moteur ni de transport nouveau.
