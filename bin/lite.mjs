...
export function skillFrontmatter(text){const front=text.match(/^---\n([\s\S]*?)\n---\n/);const name=front&&/^name: (\S+)$/m.exec(front[1])?.[1],description=front&&/^description: (.+)$/m.exec(front[1])?.[1];if(!name||!description)throw new Error('SKILL.md : frontmatter YAML avec `name` et `description` sur une ligne attendu.');return {name,description};}
