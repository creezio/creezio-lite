...
export async function inspectOrchestration(app){
 const sources=await orchestrationSources();for(const path of [...Object.keys(sources),orchestrationManifest])await assertConfined(app,path);
 const manifest=await readManifest(app),files_={},conflicts=[],pending=[];
