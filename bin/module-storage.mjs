...
export async function scaffoldModuleStorage(app,schema,{table:tableName,columns:columnMap,legacyNullable}={}){
 const table=tableName??'mod_'+schema.id.replaceAll('-','_'),spec={schema,storage:{kind:'relational',table,...(columnMap?{columns:columnMap}:{}),...(legacyNullable?{legacyNullable}:{})}};
 ...
 for(const field of storedFields(schema)){const name=columnMap?.[field.key]??field.key;columns[name]=column(name,field.type==='number'?(field.integer?'integer':'real'):field.type==='boolean'?'integer':'text',!!field.required&&!legacyNullable?.includes(field.key));}
 ...
 return {table,...(columnMap?{columns:columnMap}:{}),...(legacyNullable?{legacyNullable}:{}),migration:{id:'mod_'+schema.id.replaceAll('-','_')+'_initial',file:'drizzle/'+tag+'.sql',tables:[table]}};
}
