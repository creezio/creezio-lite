...
export async function scaffoldModuleStorage(app,schema,{table:tableName,columns:columnMap}={}){
 const table=tableName??'mod_'+schema.id.replaceAll('-','_'),spec={schema,storage:{kind:'relational',table,...(columnMap?{columns:columnMap}:{})}};
 ...
 for(const field of storedFields(schema)){const name=columnMap?.[field.key]??field.key;columns[name]=column(name,field.type==='number'?(field.integer?'integer':'real'):field.type==='boolean'?'integer':'text',!!field.required);}
 ...
 return {table,...(columnMap?{columns:columnMap}:{}),migration:{id:'mod_'+schema.id.replaceAll('-','_')+'_initial',file:'drizzle/'+tag+'.sql',tables:[table]}};
}
