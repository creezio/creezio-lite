...
export async function scaffoldModuleStorage(app,schema,{table:tableName}={}){
 ...
 const declaration=Object.values(columns).map(c=>' '+JSON.stringify(c.name)+':'+c.type+'('+JSON.stringify(c.name)+')'+(c.primaryKey?'.primaryKey()':'')+(c.notNull?'.notNull()':'')+(c.default!==undefined?'.default('+c.default+')':'')+(c.name==='org_id'?'.references(()=>organizations.id)':'')).join(',\n');
 const source=`// Module-owned relational schema; change it through a new additive migration.\nimport {sqliteTable,text,integer,real,index,check} from 'drizzle-orm/sqlite-core';\nimport {sql} from 'drizzle-orm';\nimport {organizations} from '../../../db/schema';\nexport const ${table}=sqliteTable('${table}',{\n${declaration}\n},t=>[index('${indexName}').on(t.org_id,t.deleted_at,t.updated_at),check('${checkName}',sql\`\${t.version} >= 1\`)]);\n`;
