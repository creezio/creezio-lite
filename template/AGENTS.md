...
14. Les missions de développement suivent le standard d’orchestration `.cursor/skills/lite-orchestration/SKILL.md` (Astra orchestre et décide, Cursor réalise ; sélection du modèle choisie une fois à l’attribution, Fable 5.1 par défaut ; brief et réception compacts). Ces fichiers et `manifest.json` sont gérés par le kit : les mettre à jour par `node bin/lite.mjs adopt --app <dossier> --apply` depuis la version validée du kit, jamais à la main. Ce fichier AGENTS.md et les règles métier restent la propriété de l’application.
