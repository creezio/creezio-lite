...
export function creezioUiPreview(): Plugin {
  return { name:'creezio-ui-preview', enforce:'pre', apply:'serve',
    configureServer(server){
      server.middlewares.use(async(req,res,next)=>{
        const path=req.url?.split('?')[0];
        if(path!=='/__creezio-ui-check'&&path!=='/__creezio-ui-check/frame')return next();
        try{
