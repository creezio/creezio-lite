...
/** Resolve the actual Creezio source packages. No generated replacement UI. */
export function creezioSource(): Plugin {
  ...
  return {
    name: "creezio-source-packages",
    enforce: "pre",
    resolveId(id, importer) {
      if (importer?.startsWith(packages) && ["recharts", "react-resizable-panels"].includes(id)) {
        return this.resolve(id === "recharts" ? "creezio-recharts" : "creezio-resizable-panels", resolve("app/page.tsx"), { skipSelf: true });
      }
      if (id === "@creezio-lite/core" || id.startsWith("@creezio-lite/core/")) return source(resolve("creezio/core", id.slice("@creezio-lite/core/".length) || "index.ts"));
      if (id.startsWith("@creezio/")) {
        const [name, ...parts] = id.slice("@creezio/".length).split("/");
